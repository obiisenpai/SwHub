...
-- ================================================================
-- SWITCH HUB UI LIBRARY  (thay thế redz-V5-remake)
-- Style: Switch Hub Premium | Vision 2.1
-- API tương thích: MakeWindow, MakeTab, AddToggle, AddButton,
--   AddDropdown, AddSlider, AddSection, AddParagraph, AddTextBox,
--   AddDiscordInvite, AddMinimizeButton, SetScale
-- ================================================================
local function MakeSwitchHubLibrary()
    local lib = {}
    ...
    -- Remove old instance to prevent duplicates on re-run
    for _, v in pairs(CoreGui:GetChildren()) do
        if v.Name == "SwitchHubUI" then v:Destroy() end
    end

    local Screen = Instance.new("ScreenGui")
    Screen.Name            = "SwitchHubUI"
    Screen.ResetOnSpawn    = false
    Screen.ZIndexBehavior  = Enum.ZIndexBehavior.Sibling
    Screen.Parent          = CoreGui

    -- ── Toggle / Avatar button (bottom-left) ────────────────────
    local ToggleBtn = Instance.new("ImageButton")
    ToggleBtn.Name                = "ToggleAvatar"
    ToggleBtn.Parent              = Screen
    ToggleBtn.BackgroundColor3    = Color3.fromRGB(255,255,255)
    ToggleBtn.Position            = UDim2.new(0, 15, 1, -75)
    ToggleBtn.Size                = UDim2.new(0, 55, 0, 55)
    ToggleBtn.Image               = "rbxassetid://16060333448"
    ToggleBtn.ZIndex              = 10
    Instance.new("UICorner", ToggleBtn).CornerRadius = UDim.new(1, 0)
    local tbStroke = Instance.new("UIStroke", ToggleBtn)
    tbStroke.Color     = Color3.fromRGB(255, 40, 40)
    ...
    MF.Position              = UDim2.new(0.5, -380, 0.5, -220)
    MF.Size                  = UDim2.new(0, 760, 0, 440)
    MF.Active                = true
    MF.Draggable             = true
    Instance.new("UICorner", MF).CornerRadius = UDim.new(0, 10)
    local mfStroke = Instance.new("UIStroke", MF)
    mfStroke.Color     = Color3.fromRGB(255, 35, 35)
    mfStroke.Thickness = 1.5

    -- Header bar
    local HeaderBar = Instance.new("Frame", MF)
    HeaderBar.BackgroundColor3      = Color3.fromRGB(20, 0, 0)
    ...
    TitleLbl.Text     = "Switch Hub | Premium"
    TitleLbl.TextColor3       = Color3.fromRGB(255, 55, 55)
    ...
    VerLbl.Text     = "Vision 2.1"
    VerLbl.TextColor3     = Color3.fromRGB(255, 120, 120)
    VerLbl.TextSize       = 11
    VerLbl.TextXAlignment = Enum.TextXAlignment.Right

    -- Separator line
    local Sep = Instance.new("Frame", MF)
    Sep.BackgroundColor3      = Color3.fromRGB(255, 35, 35)
    Sep.BackgroundTransparency = 0.6
    Sep.Position = UDim2.new(0, 10, 0, 46)
    Sep.Size     = UDim2.new(1, -20, 0, 1)

    -- ── Sidebar (tabs list) ──────────────────────────────────────
    local Sidebar = Instance.new("ScrollingFrame", MF)
    Sidebar.BackgroundTransparency = 1
    Sidebar.Position               = UDim2.new(0, 5, 0, 52)
    Sidebar.Size                   = UDim2.new(0, 118, 1, -60)
    Sidebar.ScrollBarThickness     = 2
    Sidebar.ScrollBarImageColor3   = Color3.fromRGB(255, 40, 40)
    ...
    -- ── Pages container ──────────────────────────────────────────
    local PagesCon = Instance.new("Frame", MF)
    PagesCon.BackgroundTransparency = 1
    PagesCon.Position               = UDim2.new(0, 130, 0, 52)
    PagesCon.Size                   = UDim2.new(1, -138, 1, -60)
    ...
    local function ToggleUI()
        ...
        MF.Visible = uiVisible
    end
    ToggleBtn.MouseButton1Click:Connect(ToggleUI)
    CloseBtn.MouseButton1Click:Connect(ToggleUI)

    -- Tab tracking
    local Tabs      = {}  -- {btn, page, stroke}
    local ActivePage = nil

    local function ActivateTab(idx)
        for i, t in ipairs(Tabs) do
            if i == idx then
                t.btn.BackgroundTransparency = 0.35
                t.stroke.Color              = Color3.fromRGB(255, 40, 40)
                t.btn.TextColor3            = Color3.fromRGB(255, 200, 200)
                t.page.Visible              = true
                ActivePage = t.page
            else
                t.btn.BackgroundTransparency = 0.82
                t.stroke.Color              = Color3.fromRGB(38, 38, 38)
                t.btn.TextColor3            = Color3.fromRGB(180, 130, 140)
                t.page.Visible              = false
            end
        end
    end

    -- ── Helper: shared element sizing ────────────────────────────
    local ELEM_HEIGHT = 30
    local ELEM_COLOR  = Color3.fromRGB(22, 22, 22)

    local function MakeElemFrame(parent)
        local f = Instance.new("Frame")
        f.Parent              = parent
        f.BackgroundColor3    = ELEM_COLOR
        f.BackgroundTransparency = 0.5
        f.Size                = UDim2.new(1, -4, 0, ELEM_HEIGHT)
        Instance.new("UICorner", f).CornerRadius = UDim.new(0, 6)
        return f
    end

    local function MakeLabel(parent, text, xAlign)
    ...
    -- ── Window object (returned by MakeWindow) ───────────────────
    local winObj = {}
    setmetatable(winObj, {__index = lib})

    function winObj:AddMinimizeButton(_cfg)  -- ignore, we have ToggleBtn
    end

    function winObj:MakeTab(cfg)
        local tabName = (type(cfg) == "table") and (cfg[1] or "Tab") or tostring(cfg)

        -- Sidebar button
        local TabBtn = Instance.new("TextButton")
        TabBtn.Parent                = Sidebar
        TabBtn.BackgroundColor3      = Color3.fromRGB(22, 0, 0)
        TabBtn.BackgroundTransparency = 0.82
        TabBtn.Size                  = UDim2.new(1, -8, 0, 34)
        TabBtn.Font                  = Enum.Font.GothamBold
        TabBtn.Text                  = tabName
        TabBtn.TextColor3            = Color3.fromRGB(180, 130, 140)
        TabBtn.TextSize              = 12
        TabBtn.TextWrapped           = true
        Instance.new("UICorner", TabBtn).CornerRadius = UDim.new(0, 7)
        local tbS = Instance.new("UIStroke", TabBtn)
        tbS.Color     = Color3.fromRGB(38, 38, 38)
        tbS.Thickness = 1

        -- Page ScrollingFrame
        local Page = Instance.new("ScrollingFrame")
        Page.Name                  = tabName
        Page.Parent                = PagesCon
        Page.BackgroundTransparency = 1
        Page.Size                  = UDim2.new(1, 0, 1, 0)
        Page.Visible               = false
        Page.ScrollBarThickness    = 3
        Page.ScrollBarImageColor3  = Color3.fromRGB(255, 40, 40)
        ...
        local idx = #Tabs + 1
        table.insert(Tabs, {btn = TabBtn, page = Page, stroke = tbS})
        ...
        function tab:AddSection(cfg2)
            local sName = (type(cfg2) == "table") and (cfg2[1] or "") or tostring(cfg2)
            local sf = Instance.new("Frame")
            sf.Parent              = Page
            sf.BackgroundColor3    = Color3.fromRGB(255, 35, 35)
            sf.BackgroundTransparency = 0.65
            sf.Size                = UDim2.new(1, -4, 0, 22)
            ...
            sl.Text      = sName
            sl.TextColor3 = Color3.fromRGB(255, 200, 200)
            sl.TextSize   = 11
        ...
        function tab:AddParagraph(cfg2)
            ...
            local pContent = cfg2.Content or cfg2.Desc or ""

            local pf = Instance.new("Frame")
            pf.Parent              = Page
            pf.BackgroundColor3    = ELEM_COLOR
            pf.BackgroundTransparency = 0.55
            pf.Size                = UDim2.new(1, -4, 0, 38)
            pf.AutomaticSize       = Enum.AutomaticSize.Y
            Instance.new("UICorner", pf).CornerRadius = UDim.new(0, 6)

            local pfl = Instance.new("UIListLayout", pf)
            pfl.SortOrder = Enum.SortOrder.LayoutOrder

            local ptL = Instance.new("TextLabel", pf)
            ptL.BackgroundTransparency = 1
            ptL.Size   = UDim2.new(1, -10, 0, 19)
            ptL.Font   = Enum.Font.GothamBold
            ptL.Text   = "  " .. pTitle
            ptL.TextColor3 = Color3.fromRGB(255, 160, 160)
            ptL.TextSize   = 12
            ptL.TextXAlignment = Enum.TextXAlignment.Left

            local pcL = Instance.new("TextLabel", pf)
            pcL.BackgroundTransparency = 1
            pcL.Size         = UDim2.new(1, -10, 0, 18)
            pcL.AutomaticSize = Enum.AutomaticSize.Y
            pcL.Font         = Enum.Font.Gotham
            pcL.Text         = "  " .. pContent
            pcL.TextColor3   = Color3.fromRGB(175, 175, 175)
            pcL.TextSize     = 11
            pcL.TextWrapped  = true
            pcL.TextXAlignment = Enum.TextXAlignment.Left

            local paraObj = {}
            function paraObj:Set(txt)
                pcL.Text = "  " .. tostring(txt)
            end
            function paraObj:SetDesc(txt)
                pcL.Text = "  " .. tostring(txt)
            end
            return paraObj
        end
        ...
        function tab:AddToggle(cfg2)
            local name     = cfg2.Name or cfg2.Title or "Toggle"
            local default  = cfg2.Default ~= nil and cfg2.Default or (cfg2.Value ~= nil and cfg2.Value or false)
            local callback = cfg2.Callback

            local f = MakeElemFrame(Page)

            local lbl = MakeLabel(f, name)
            lbl.Size = UDim2.new(1, -60, 1, 0)

            local SBg = Instance.new("TextButton", f)
            SBg.BackgroundColor3 = default and Color3.fromRGB(220, 40, 40) or Color3.fromRGB(45, 45, 45)
            SBg.Position = UDim2.new(1, -42, 0.5, -8)
            SBg.Size     = UDim2.new(0, 32, 0, 16)
            SBg.Text     = ""
            Instance.new("UICorner", SBg).CornerRadius = UDim.new(1, 0)

            local Circ = Instance.new("Frame", SBg)
            Circ.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
            Circ.Position = default and UDim2.new(1, -14, 0.5, -6) or UDim2.new(0, 2, 0.5, -6)
            ...
            SBg.MouseButton1Click:Connect(function()
                ...
                TweenService:Create(SBg,  TweenInfo.new(0.18), {
                    BackgroundColor3 = state and Color3.fromRGB(220,40,40) or Color3.fromRGB(45,45,45)
                }):Play()
                TweenService:Create(Circ, TweenInfo.new(0.18), {
        ...
        function tab:AddButton(cfg2)
            local name     = cfg2.Name or cfg2.Title or "Button"
            local callback = cfg2.Callback

            local btn = Instance.new("TextButton", Page)
            btn.BackgroundColor3      = Color3.fromRGB(180, 25, 25)
            btn.BackgroundTransparency = 0.3
            btn.Size   = UDim2.new(1, -4, 0, ELEM_HEIGHT)
        ...
        function tab:AddDropdown(cfg2)
            local name     = cfg2.Name or cfg2.Description or "Dropdown"
            local options  = cfg2.Options or {}
            local default  = cfg2.Default or options[1]
            local callback = cfg2.Callback

            if callback and default ~= nil then pcall(callback, default) end

            local OPT_H   = 24
            local baseH   = ELEM_HEIGHT + 4

            local Container = Instance.new("Frame", Page)
            Container.BackgroundColor3    = ELEM_COLOR
            Container.BackgroundTransparency = 0.5
            Container.Size  = UDim2.new(1, -4, 0, baseH)
            Container.ClipsDescendants = false
            Instance.new("UICorner", Container).CornerRadius = UDim.new(0, 6)

            local lbl = MakeLabel(Container, name)
            lbl.Size = UDim2.new(0.55, 0, 0, baseH)
            ...
            DropBtn.Position  = UDim2.new(1, -138, 0.5, -12)
            DropBtn.Size      = UDim2.new(0, 128, 0, 24)
            DropBtn.Font      = Enum.Font.Gotham
            DropBtn.Text      = tostring(default or "Select") .. "  ▾"
            DropBtn.TextColor3 = Color3.fromRGB(255, 155, 175)
            DropBtn.TextSize   = 11
            DropBtn.TextTruncate = Enum.TextTruncate.AtEnd
            Instance.new("UICorner", DropBtn).CornerRadius = UDim.new(0, 5)
            Instance.new("UIStroke", DropBtn).Color = Color3.fromRGB(55, 55, 55)
            ...
            DropList.Position  = UDim2.new(1, -140, 0, baseH)
            DropList.Size      = UDim2.new(0, 132, 0, #options * OPT_H)
            DropList.Visible   = false
            DropList.ZIndex    = 5
            Instance.new("UICorner", DropList).CornerRadius = UDim.new(0, 5)
            Instance.new("UIStroke", DropList).Color = Color3.fromRGB(255, 35, 35)
        ...
        function tab:AddSlider(cfg2)
            local name     = cfg2.Name or cfg2.Title or "Slider"
            local minV     = cfg2.Min or 0
            local maxV     = cfg2.Max or 100
            local step     = cfg2.Increase or 1
            local default  = cfg2.Default ~= nil and cfg2.Default or minV
            local callback = cfg2.Callback
            ...
            nameLbl.Text     = name .. ":  " .. tostring(default)
            ...
            Track.Size     = UDim2.new(1, -20, 0, 9)
            Instance.new("UICorner", Track).CornerRadius = UDim.new(1, 0)

            local ratio0 = math.clamp((default - minV) / (maxV - minV), 0, 1)
            ...
            UIS.InputChanged:Connect(function(inp)
                if dragging and inp.UserInputType == Enum.UserInputType.MouseMovement then
                    ...
                    nameLbl.Text  = name .. ":  " .. tostring(curVal)
        ...
        function tab:AddTextBox(cfg2)
            local name        = cfg2.Name or "TextBox"
            local placeholder = cfg2.PlaceholderText or ""
            local callback    = cfg2.Callback
            ...
            box.Position  = UDim2.new(0, 10, 0, 27)
        ...
        -- AddDiscordInvite ────────────────────────────────────────
        function tab:AddDiscordInvite(cfg2)
            local dname  = cfg2.Name or "Discord"
            local desc   = cfg2.Description or ""
            local invite = cfg2.Invite or ""

            local f = Instance.new("Frame", Page)
            f.BackgroundColor3      = Color3.fromRGB(58, 75, 210)
            ...
            nl.Position = UDim2.new(0, 52, 0, 7)
            ...
            dl.Position = UDim2.new(0, 52, 0, 29)
            dl.Size     = UDim2.new(1, -60, 0, 16)
            dl.Font     = Enum.Font.Gotham
            dl.Text     = desc ~= "" and desc or invite
            dl.TextColor3 = Color3.fromRGB(200, 210, 255)
            dl.TextSize   = 10
    ...
    -- ── lib.SetScale (called as vu32:SetScale()) ─────────────────
    function lib:SetScale(scale)
        if scale <= 350 then
            MF.Size = UDim2.new(0, 920, 0, 520)
        elseif scale >= 800 then
            MF.Size = UDim2.new(0, 600, 0, 340)
        else
            MF.Size = UDim2.new(0, 760, 0, 440)
        end
    end

    -- Footer label (always visible at bottom)
    local FooterLbl = Instance.new("TextLabel", MF)
    FooterLbl.BackgroundTransparency = 1
    FooterLbl.AnchorPoint = Vector2.new(1, 1)
    FooterLbl.Position    = UDim2.new(1, -10, 1, -4)
    FooterLbl.Size        = UDim2.new(0, 220, 0, 14)
    FooterLbl.Font        = Enum.Font.Gotham
    FooterLbl.Text        = "Switch Hub | Vision 2.1 Premium"
    FooterLbl.TextColor3  = Color3.fromRGB(255, 90, 90)
    FooterLbl.TextSize    = 10
    FooterLbl.TextXAlignment = Enum.TextXAlignment.Right

    return lib
end
-- END SWITCH HUB LIBRARY
local vu32 = MakeSwitchHubLibrary()
